...
from modules.schemas import SourceTask
...
def test_load_from_cli_rejects_deprecated_objects_env_id_field(tmp_path: Path) -> None:
    task_file = tmp_path / "tasks.yaml"
    task_file.write_text(
        '\n'.join(
            [
                '- task_id: "1"',
                '  objects_env_id: "legacy-env"',
                '  instruction: "Pick up the tallest bottle."',
            ]
        ),
        encoding="utf-8",
    ...
    with pytest.raises(ValueError, match="deprecated field: objects_env_id"):
        TaskLoader().load_from_cli(task_file=task_file, task_id="1")
