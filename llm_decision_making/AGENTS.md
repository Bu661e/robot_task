...
- 跨模块共享协议文档统一放在仓库根目录 `docs/protocols/`，包含与 `robot_service`、`perception_service` 的 HTTP 交互协议说明。
- `llm_decision_making/docs/` 目录保留本模块内部设计、计划和其他本地说明文档。
...
- 新增跨模块接口协议文档时，统一放到仓库根目录 `docs/protocols/`，不要再放到单个模块目录下。
...
- 在编写接口协议代码前，先检查仓库根目录 `docs/protocols/对应接口文档` 与当前任务是否一致，如果不一致询问是否更改对应协议文档或更改任务。
