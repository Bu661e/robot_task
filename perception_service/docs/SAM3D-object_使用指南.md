...
本文档面向当前仓库里的 `perception_service` 开发，整理 `SAM3D-object` 仓库的作用、安装方式、入口代码、输入输出约束，以及接入我们感知服务时应该怎么用。

## 1. 本地落位信息

- 上游仓库：`https://github.com/facebookresearch/sam-3d-objects`
- 本地路径：`perception_service/SAM3D-object/`
...
也就是说，在我们的 `perception_service` 里，更合理的职责分工是：
...
```bash
cd perception_service/SAM3D-object
...
所以如果要接进 `perception_service` 的 HTTP 响应，不能直接把整个 `output` 原样返回。
...
## 10. 适合接进当前 `perception_service` 的方式
...
如果下一步要真正实现 `perception_service`，建议按这个优先级来：
