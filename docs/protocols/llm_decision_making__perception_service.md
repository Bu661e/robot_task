...
- `pointmap` 固定由 `perception_service` 基于 `depth + camera_intrinsics` 在服务内部生成，不作为决策端上传字段
...
- 接收决策端上传的观测文件，例如 RGB 图、深度图、已有 mask 等
...
### 2.6 Observation 必须足够支撑 3D 感知，且 pointmap 在感知侧内部生成

当前协议要求：

- `rgb_artifact_id` 必填
- `depth_artifact_id` 必填
- `camera_intrinsics` 必填

当前固定约定：

- `llm_decision_making` 不上传 `pointmap_artifact_id`
- `perception_service` 收到 `depth_artifact_id` 后，结合 `camera_intrinsics` 在服务内部生成 `pointmap`
- 生成出的 `pointmap` 视为感知链路内部中间结果，不作为当前跨模块协议的输入字段
...
- `depth_artifact_id` 必填
- `camera_intrinsics` 必填
- 当前协议不接受决策端上传 `pointmap_artifact_id`
...
- `pointmap` 是当前感知链路里的内部中间结果，不是决策端上传 artifact 类型
- 当前协议下：
  - `llm_decision_making` 只上传 RGB、深度图和相机内参
  - `perception_service` 在服务内部把深度图恢复为 `pointmap`
  - 内部 `pointmap` 直接供 3D 感知模型使用
- 因此当前版本不定义 `artifact_type = "pointmap"` 的跨模块上传格式
...
- `MISSING_GEOMETRY_SOURCE`
  - `depth_artifact_id` 缺失，无法在感知侧生成内部 `pointmap`
- `SAM3_NO_MASK`
