...
这里的机器人侧是一个独立于 `llm_decision_making/main.py` 的 Isaac Sim worker 进程，职责是：

- 启动 Isaac Sim 5.0.0 运行时
- 加载固定基础环境与可切换的桌面场景
- 提供本地 HTTP 接口给 `llm_decision_making/modules/robot_bridge.py`
...
- `llm_decision_making/main.py` 已能执行到 `M1 task_parser`
...
- 在主进程侧增加了 `llm_decision_making/modules/robot_bridge.py`
- `robot_bridge` 已支持：
  - 启动 worker 子进程
  - 轮询 `/health`
  - 请求 `/capture_frame`
...
- `llm_decision_making/main.py` 已增加：
  - `--scene-id`
  - `--enable-robot-worker`
  - `--robot-headless`
- 当前默认 **不会** 自动启动 worker。
...
4. 补齐 `pick_and_place`
5. 再把 `llm_decision_making/main.py` 的 robot worker 开关真正打开做链路测试
