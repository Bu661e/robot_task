...
它和主进程解耦，主进程只通过 `llm_decision_making/modules/robot_bridge.py` 和本地 HTTP 协议访问它。
...
1. 主进程创建本轮输出目录
2. 主进程调用 `llm_decision_making.modules.robot_bridge.start_worker(...)`
...
5. 最后再打开 `llm_decision_making/main.py` 里的 robot worker 运行测试
